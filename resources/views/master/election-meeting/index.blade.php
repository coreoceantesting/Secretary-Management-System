<x-admin.layout>
    <x-slot name="title">Election Meeting(निवडणूक बैठक)</x-slot>
    <x-slot name="heading">Election Meeting(निवडणूक बैठक)</x-slot>
                ...
                <div class="card">
                    <form class="theme-form" name="addForm" id="addForm" enctype="multipart/form-data">
                        ...
                        <div class="card-header">
                            <h4 class="card-title">Add Election Meeting(निवडणूक बैठक जोडा)</h4>
                        </div>
                        <div class="card-body">
                            <div class="mb-3 row">
                                <div class="col-md-4">
        ...
        <div class="row" id="editContainer" style="display:none;">
            <div class="col">
                <form class="form-horizontal form-bordered" method="post" id="editForm">
                    ...
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title">Edit Election Meeting(निवडणूक बैठक संपादित करा)</h4>
                        </div>
                            ...
                            <table id="buttons-datatables" class="table table-bordered nowrap align-middle" style="width:100%">
                                <thead>
                                    <tr>
                                        <th>Sr no.</th>
                                        <th>Name</th>
                                        <th>Head Person name</th>
                                        <th>Head Person Designation</th>
                                        <th>Action</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($electionMeetings as $electionMeeting)
                                        <tr>
                                            ...
                                            <td>{{ $electionMeeting->name }}</td>
                                            <td>{{ $electionMeeting->head_person_name }}</td>
                                            <td>{{ $electionMeeting->head_person_designation }}</td>
                                            <td>
                                                @can('meeting.edit')
                                                <button class="edit-element btn text-secondary px-2 py-1" title="Edit Election Meeting" data-id="{{ $electionMeeting->id }}"><i data-feather="edit"></i></button>
                                                @endcan
                                                @can('meeting.delete')
                                                <button class="btn text-danger rem-element px-2 py-1" title="Delete Election Meeting" data-id="{{ $electionMeeting->id }}"><i data-feather="trash-2"></i> </button>
                                                @endcan
                                            </td>
                                        </tr>
                                    @endforeach
...
<script>
    $("#addForm").submit(function(e) {
        ...
        $.ajax({
            url: '{{ route('master.election-meeting.store') }}',
            type: 'POST',
            data: formdata,
            contentType: false,
            processData: false,
            ...
            success: function(data)
            {
                ...
                if (!data.error)
                    swal("Successful!", data.success, "success")
                        .then((action) => {
                            window.location.href = '{{ route('master.election-meeting.index') }}';
                        });
                else
...
<script>
    $("#buttons-datatables").on("click", ".edit-element", function(e) {
        ...
        var url = "{{ route('master.election-meeting.edit', ":model_id") }}";
        ...
        $.ajax({
            ...
            success: function(data, textStatus, jqXHR) {
                ...
                if (!data.error)
                {
                    $("#editForm input[name='edit_model_id']").val(data.meeting.id);
...
<script>
    $(document).ready(function() {
        $("#editForm").submit(function(e) {
            ...
            var url = "{{ route('master.election-meeting.update', ":model_id") }}";
            //
            $.ajax({
                url: url.replace(':model_id', model_id),
                type: 'POST',
...
<script>
    $("#buttons-datatables").on("click", ".rem-element", function(e) {
        ...
        .then((justTransfer) =>
        {
            ...
            {
                var model_id = $(this).attr("data-id");
                var url = "{{ route('master.election-meeting.destroy', ":model_id") }}";
