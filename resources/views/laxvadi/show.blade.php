<x-admin.layout>
    ...
    <div class="row" id="addContainer">
        <div class="col-sm-12">
            <div class="card">
                <form class="theme-form" method="post" action="{{ route('laxvadi.response') }}" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="id" value="{{ $question->id }}">
                    <div class="card-header">
                        <h4 class="card-title">Laxvedhi(लक्षवेधी)</h4>
                    </div>
                    <div class="card-body">
                        <div class="mb-3 row">
                            <div class="table-responsive">
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <th>Laxvedhi File(लक्षवेधी फाइल)</th>
                    ...
                    <div class="card-footer">
                        ...
                        <a href="{{ route('laxvadi.index') }}" class="btn btn-warning">Cancel</a>
                    </div>
                    @endcan
                </form>
            </div>
            ...
            $(document).ready(function(){
                $('body').on('click', '.sendQuestionResponse', function(){
                    ...
                    if(userConfirmation ){
                        ...
                        $.ajax({
                            url: "{{ route('laxvadi.saveSingleResponse') }}",
                            type: 'POST',
                            data: {
                                _token: "{{ csrf_token() }}",
                                id: id,
            ...
            $(document).ready(function(){
                $('body').on('click', '.acceptQuestion', function(){
                    ...
                    if(userConfirmation ){
                        ...
                        $.ajax({
                            url: "{{ route('laxvadi.acceptQuetionByMayor') }}",
                            type: 'POST',
                            data: {
                                _token: "{{ csrf_token() }}",
                                id: id
        ...
        <script>
            $(document).ready(function(){
                $('body').on('click', '.sendQuestion', function(){
                    ...
                    let userConfirmation  = confirm('Are you sure you want to send this laxvedhi to department');

                    if(userConfirmation ){
                        let id = $(this).closest('tr').find('.questionId').val();
                        $(this).attr('id', 'sendButtonQuestion');
                        $.ajax({
                            url: "{{ route('laxvadi.sendQuestion') }}",
                            type: 'POST',
                            data: {
                                _token: "{{ csrf_token() }}",
                                id: id
