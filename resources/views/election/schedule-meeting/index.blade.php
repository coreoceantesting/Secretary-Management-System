            ...
            <div class="card">
                <form class="theme-form" name="addForm" id="addForm" enctype="multipart/form-data">
                    ...
                    <div class="card-header">
                        <h4 class="card-title">Add Election Schedule Meeting(निवडणूक वेळापत्रक बैठक जोडा)</h4>
                    </div>
                    <div class="card-body">
                        <div class="mb-3 row">
                            <div class="col-md-4">
                                <label class="col-form-label" for="election_agenda_id">Select Agenda(अजेंडा निवडा) <span class="text-danger">*</span></label>
                                <select class="form-select col-sm-12" id="election_agenda_id" name="election_agenda_id" required>
                            ...
                            <input type="hidden" name="election_meeting_id" id="hiddenMeetingId">
                            <div class="col-md-4">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="buttons-datatables" class="table table-bordered nowrap align-middle" style="width:100%">
                            ...
                            <tbody>
                                @foreach ($electionScheduleMeetings as $electionScheduleMeeting)
                                    <tr>
...
<script>
    $("#addForm").submit(function(e) {
        ...
        $.ajax({
            url: '{{ route('election.schedule-meeting.store') }}',
            type: 'POST',
            data: formdata,
            contentType: false,
            processData: false,
            ...
            success: function(data)
            {
                ...
                if (!data.error)
                    swal("Successful!", data.success, "success")
                        .then((action) => {
                            window.location.href = '{{ route('election.schedule-meeting.index') }}';
                        });
                else
...
<script>
    $("#buttons-datatables").on("click", ".edit-element", function(e) {
        ...
        var url = "{{ route('election.schedule-meeting.edit', ":model_id") }}";
...
<script>
    $(document).ready(function() {
        $("#editForm").submit(function(e) {
            ...
            var url = "{{ route('election.schedule-meeting.update', ":model_id") }}";
            //
            $.ajax({
                url: url.replace(':model_id', model_id),
                type: 'POST',
...
<script>
    $("#buttons-datatables").on("click", ".rem-element", function(e) {
        ...
        .then((justTransfer) =>
        {
            ...
            {
                var model_id = $(this).attr("data-id");
                var url = "{{ route('election.schedule-meeting.destroy', ":model_id") }}";
...
<script>
    ...
    $("body").on("click", ".rem-cancel", function(e) {
        ...
        var url = "{{ route('election.schedule-meeting.cancel', ":model_id") }}";
...
<script>
    $(document).ready(function(){
        $('#election_agenda_id').change(function(){
            let agendaId = $(this).val();
            $.ajax({
                url: "{{ route('election.schedule-meeting.fetch-agenda') }}",
                type: 'POST',
                data: {
                    '_method': "get",
                    'election_agenda_id': agendaId,
                },
                beforeSend: function()
                {
                    $('#preloader').css('opacity', '0.5');
                ...
                success: function(data, textStatus, jqXHR) {
                    if (!data.error && !data.error2) {
                        $("#addForm select[name='election_meeting_id']").val(data.agenda.meeting_id);
