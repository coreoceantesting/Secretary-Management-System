<x-admin.layout>
    <x-slot name="title">Election Reschedule Meeting(निवडणुकीच्या वेळापत्रकाची बैठक)</x-slot>
    <x-slot name="heading">Election Reschedule Meeting(निवडणुकीच्या वेळापत्रकाची बैठक)</x-slot>
            ...
            <div class="card">
                <form class="theme-form" name="addForm" id="addForm" enctype="multipart/form-data">
                    ...
                    <div class="card-header">
                        <h4 class="card-title">Add Election Reschedule Meeting(निवडणुकीच्या वेळापत्रकाची बैठक जोडा)</h4>
                    </div>
                    <div class="card-body">
                        <div class="mb-3 row">
                            <div class="col-md-4">
                                <label class="col-form-label" for="election_meeting_id">Select Meeting(मीटिंग निवडा) <span class="text-danger">*</span></label>
                                <select class="form-select col-sm-12 selectMeetingId" required id="election_meeting_id" name="election_meeting_id">
    ...
    <div class="row" id="editContainer" style="display:none;">
        <div class="col">
            <form class="form-horizontal form-bordered" method="post" id="editForm">
                ...
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">Edit Election Reschedule Meeting</h4>
                    </div>
    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                @can('election-reschedule-meeting.create')
                <div class="card-header">
                    <div class="row">
                        <div class="col-sm-6">
                            <div class="">
                            ...
                            <tbody>
                                @foreach ($rescheduleMeetings as $rescheduleMeeting)
                                    <tr>
                                        ...
                                        <td>
                                            @can('election-reschedule-meeting.show')
                                            <a href="{{ route('election.reschedule-meeting.show', $rescheduleMeeting->id) }}" class="btn text-secondary px-1 py-1">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-eye"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>
                                            </a>
                                            @endcan

                                            @can('election-reschedule-meeting.cancel')
...
<script>
    $("#addForm").submit(function(e) {
        ...
        $.ajax({
            url: '{{ route('election.reschedule-meeting.store') }}',
            type: 'POST',
            data: formdata,
            contentType: false,
            processData: false,
            ...
            success: function(data)
            {
                ...
                if (!data.error)
                    swal("Successful!", data.success, "success")
                        .then((action) => {
                            window.location.href = '{{ route('election.reschedule-meeting.index') }}';
                        });
                else
...
<script>
    $("#buttons-datatables").on("click", ".edit-element", function(e) {
        ...
        var url = "{{ route('election.reschedule-meeting.edit', ":model_id") }}";
...
<script>
    $(document).ready(function() {
        $("#editForm").submit(function(e) {
            ...
            var url = "{{ route('election.reschedule-meeting.update', ":model_id") }}";
            //
            $.ajax({
                url: url.replace(':model_id', model_id),
                type: 'POST',
...
<script>
    $("#buttons-datatables").on("click", ".rem-element", function(e) {
        ...
        .then((justTransfer) =>
        {
            ...
            {
                var model_id = $(this).attr("data-id");
                var url = "{{ route('election.reschedule-meeting.destroy', ":model_id") }}";
...
<script>
    $(document).ready(function(){
        ...
        function getScheduleMeeting(id){
            var url = "{{ route('election.reschedule-meeting.getScheduleMeeting', ':model_id') }}";
            $.ajax({
                ...
                success: function(data) {
                    if(data.status == 200){
                        let html = `<label class="col-form-label" for="election_schedule_meeting_id">Select Schedule Meeting Date(शेड्यूल मीटिंग तारीख निवडा) <span class="text-danger">*</span></label>
                                    <select class="form-select col-sm-12 selectChnageScheduleMeetingDetails" id="election_schedule_meeting_id" name="election_schedule_meeting_id" required>
                        ...
                        html += `</select>
                                    <span class="text-danger is-invalid election_schedule_meeting_id_err"></span>`;
        ...
        function getScheduleMeetingDetails(id){
            var url = "{{ route('election.reschedule-meeting.getScheduleMeetingDetails', ':model_id') }}";
            $.ajax({
                url: url.replace(':model_id', id),
                type: 'GET',
                contentType: false,
...
<script>
    ...
    $("body").on("click", ".rem-cancel", function(e) {
        ...
        var url = "{{ route('election.reschedule-meeting.cancel', ":model_id") }}";
