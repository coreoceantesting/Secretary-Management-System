<x-admin.layout>
                    ...
                    <div class="card-body">
                        <div class="mb-3 row">
                            <div class="col-md-4">
                                ...
                                <select name="election_meeting_id" required id="election_meeting_id" class="form-select selectMeetingId" required>
    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                @can('election-proceeding-record.create')
                <div class="card-header">
                    <div class="row">
                        <div class="col-sm-6">
                            <div class="">
                        ...
                        <table id="buttons-datatables" class="table table-bordered nowrap align-middle" style="width:100%">
                            <thead>
                                <tr>
                                    ...
                                    @can('election-proceeding-record.show')<th>Action</th>@endcan
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($proceedingRecords as $proceedingRecord)
                                    <tr>
                                        ...
                                        @can('election-proceeding-record.show')
                                        <td>
                                            <a href="{{ route('election.proceeding-record.pdf', $proceedingRecord->id) }}" class="btn btn-sm btn-primary px-2 py-1" target="_blank">
                                                Pdf
                                            </a>
                                            <a href="{{ route('election.proceeding-record.show', $proceedingRecord->id) }}" class="btn btn-sm btn-primary px-2 py-1">
                                                view
                                            </a>
                                        </td>
                                        @endcan
...
<script>
    $("#addForm").submit(function(e) {
        ...
        $.ajax({
            url: '{{ route('election.proceeding-record.store') }}',
            type: 'POST',
            data: formdata,
            contentType: false,
            processData: false,
            ...
            success: function(data)
            {
                ...
                if (!data.error)
                    swal("Successful!", data.success, "success")
                        .then((action) => {
                            window.location.href = '{{ route('election.proceeding-record.index') }}';
                        });
                else
...
<script>
    $(document).ready(function(){
        ...
        function getScheduleMeeting(id){
            var url = "{{ route('election.proceeding-record.getScheduleMeeting', ':model_id') }}";
            $.ajax({
                ...
                success: function(data) {
                    let html = `<label class="col-form-label" for="election_schedule_meeting_id">Select election schedule meeting date(शेड्यूल मीटिंग तारीख निवडा) <span class="text-danger">*</span></label>
                                <select class="form-select col-sm-12" id="election_schedule_meeting_id" name="election_schedule_meeting_id" required>
