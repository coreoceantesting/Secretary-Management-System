<x-admin.layout>
    <x-slot name="title">Election Attendance(निवडणूक उपस्थिती)</x-slot>
    <x-slot name="heading">Election Attendance(निवडणूक उपस्थिती)</x-slot>
                            ...
                            <tbody>
                                @foreach ($attendances as $attendance)
                                    <tr>
                                        ...
                                        <td>
                                            <a target="_blank" href="{{ asset('storage/'.$attendance->electionAgenda->pdf) }}" class="btn btn-primary btn-sm">View File</a>
                                        </td>
                                        <td>
                                            @php
                                            ...
                                            @if($diff <= 0)
                                            <a href="{{ route('election.attendance.show', $attendance->id) }}" class="btn btn-primary btn-sm">Mark</a>
                                            @else
