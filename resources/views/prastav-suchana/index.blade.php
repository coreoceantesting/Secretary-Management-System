<x-admin.layout>
    <x-slot name="title">Prastav Suchana(प्रस्तव सुचना)</x-slot>
    <x-slot name="heading">Prastav Suchana(प्रस्तव सुचना)</x-slot>
            ...
            <div class="card">
                <form class="theme-form" name="addForm" id="addForm" enctype="multipart/form-data">
                    ...
                    <div class="card-header">
                        <h4 class="card-title">Add Prastav Suchana(प्रस्तव सुचना जोडा)</h4>
                    </div>
                    <div class="card-body">
                        <div class="mb-3 row">
                            <div class="col-md-4">
                            ...
                            <div class="col-md-4">
                                <label class="col-form-label" for="uploadfile">Upload Prastav Suchana File(प्रस्तव सुचना फाइल अपलोड करा) <span class="text-danger">*</span></label>
                        ...
                        <table class="table table-bordered">
                            ...
                            <tbody id="addPrastavSuchana">
                                <tr id="row1">
                                    <td>
                                        <textarea class="form-control" name="question[]" placeholder="Enter Prastav Suchana" required></textarea>
                                    </td>
                                    <td>
    ...
    <div class="row" id="editContainer" style="display:none;">
        <div class="col">
            <form class="form-horizontal form-bordered" method="post" id="editForm">
                ...
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">Edit Prastav Suchana(प्रस्तव सुचना संपादित करा)</h4>
                    </div>
                    <div class="card-body py-2">
                        ...
                        <div class="mb-3 row">
                            ...
                            <div class="col-md-4 selectScheduleMeeting d-none"></div>
                            <div class="col-md-4">
                                <label class="col-form-label" for="uploadfile">Upload Prastav Suchana File(प्रस्तव सुचना फाइल अपलोड करा)</label>
                        ...
                        <table id="buttons-datatables" class="table table-bordered nowrap align-middle" style="width:100%">
                            <thead>
                                <tr>
                                    ...
                                    <th>Date</th>
                                    <th>Meeting Venue</th>
                                    <th>Prastav Suchana File</th>
                                    <th>Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($prastavSuchanas as $prastavSuchana)
                                    <tr>
                                        <td>{{ $loop->iteration }}</td>
                                        <td>{{ $prastavSuchana->department?->name ?? '-' }}</td>
                                        <td>{{ $prastavSuchana->meeting?->name ?? '-' }}</td>
                                        <td>{{ $prastavSuchana->scheduleMeeting?->unique_id ?? '-' }}</td>

                                        <td>
                                            @if($prastavSuchana->prastavSuchanaSubQuestion->whereNull('response')->count() > 0)

                                            @php
                                            $date = ($prastavSuchana->scheduleMeeting?->parentLatestScheduleMeeting?->date) ? $prastavSuchana->scheduleMeeting?->parentLatestScheduleMeeting?->date : $prastavSuchana->scheduleMeeting?->date;
                                            ...
                                            @if($daysleft <= 6 && $daysleft >=4)
                                                <div class="blink-text" style="color:#308f18!important">{{ ($prastavSuchana->scheduleMeeting?->parentLatestScheduleMeeting?->date) ? date('d-m-Y', strtotime($prastavSuchana->scheduleMeeting?->parentLatestScheduleMeeting?->date)) : date('d-m-Y', strtotime($prastavSuchana->scheduleMeeting?->date)) }}</div>
                                            @elseif($daysleft < 3)
                                            <div class="blink-text" style="color:#e92d46!important">{{ ($prastavSuchana->scheduleMeeting?->parentLatestScheduleMeeting?->date) ? date('d-m-Y', strtotime($prastavSuchana->scheduleMeeting?->parentLatestScheduleMeeting?->date)) : date('d-m-Y', strtotime($prastavSuchana->scheduleMeeting?->date)) }}</div>
                                            @else
                                            {{ ($prastavSuchana->scheduleMeeting?->parentLatestScheduleMeeting?->date) ? date('d-m-Y', strtotime($prastavSuchana->scheduleMeeting?->parentLatestScheduleMeeting?->date)) : date('d-m-Y', strtotime($prastavSuchana->scheduleMeeting?->date)) }}
                                            @endif
                                        ...
                                        <td>
                                            ...
                                            <a href="{{ route('prastav-suchana.show', $prastavSuchana->id) }}" class="btn btn-primary btn-sm">View Question</a>
                                            @endif

                                            @if($prastavSuchana->prastavSuchanaSubQuestion->where('is_mayor_selected', 1)->count() == 0)
                                            @can('prastav-suchana.edit')
                                            <button class="edit-element btn text-secondary px-2 py-1" title="Edit Question" data-id="{{ $prastavSuchana->id }}"><i data-feather="edit"></i></button>
                                            @endcan
                                            @can('prastav-suchana.delete')
                                            <button class="btn text-danger rem-element px-2 py-1" title="Delete Question" data-id="{{ $prastavSuchana->id }}"><i data-feather="trash-2"></i> </button>
                                            @endcan
                                            @endif
                                        </td>
                                    </tr>
...
<script>
    $("#addForm").submit(function(e) {
        ...
        $.ajax({
            url: '{{ route('prastav-suchana.store') }}',
            type: 'POST',
            data: formdata,
            contentType: false,
            processData: false,
            ...
            success: function(data)
            {
                ...
                if (!data.error)
                    swal("Successful!", data.success, "success")
                        .then((action) => {
                            window.location.href = '{{ route('prastav-suchana.store') }}';
                        });
                else
...
<script>
    $("#buttons-datatables").on("click", ".edit-element", function(e) {
        ...
        var url = "{{ route('prastav-suchana.edit', ":model_id") }}";
        ...
        $.ajax({
            ...
            success: function(data, textStatus, jqXHR) {
                ...
                if (!data.error)
                {
                    ...
                    // $("#editForm #department_id").html(data.prastav-suchana.);
                }
                else
                {
                    swal("Error!", data.error, "error");
...
<script>
    $(document).ready(function() {
        $("#editForm").submit(function(e) {
            ...
            var url = "{{ route('prastav-suchana.update', ":model_id") }}";
            //
            $.ajax({
                url: url.replace(':model_id', model_id),
                type: 'POST',
                ...
                success: function(data)
                {
                    ...
                    if (!data.error)
                        swal("Successful!", data.success, "success")
                            .then((action) => {
                                window.location.href = '{{ route('prastav-suchana.index') }}';
                            });
                    else
                        swal("Error!", data.error, "error");
                },
...
<script>
    $("#buttons-datatables").on("click", ".rem-element", function(e) {
        ...
        .then((justTransfer) =>
        {
            ...
            {
                var model_id = $(this).attr("data-id");
                var url = "{{ route('prastav-suchana.destroy', ":model_id") }}";
...
<script>
    $(document).ready(function(){
        ...
        function getScheduleMeeting(id){
            var url = "{{ route('prastav-suchana.getScheduleMeeting', ':model_id') }}";
            $.ajax({
        ...
        $('body').on('click', '.addMore', function(){
            ...
            $('body').find('#addPrastavSuchana').append(html);
            addcount = addcount + 1;
        });
        ...
        function getScheduleMeetingDepartments(id, type){
            var url = "{{ route('prastav-suchana.getScheduleMeetingDepartments', ':model_id') }}";
            $.ajax({
                url: url.replace(':model_id', id),
                type: 'GET',
                data: {
