<x-admin.layout>
    ...
    <div class="row" id="addContainer">
        <div class="col-sm-12">
            <div class="card">
                <form class="theme-form" method="post" action="{{ route('prastav-suchana.response') }}" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="id" value="{{ $question->id }}">
                    <div class="card-header">
                        <h4 class="card-title">Prastav Suchana(प्रस्तव सुचना)</h4>
                    </div>
                    <div class="card-body">
                        <div class="mb-3 row">
                    ...
                    <div class="card-footer">
                        ...
                        <a href="{{ route('prastav-suchana.index') }}" class="btn btn-warning">Cancel</a>
                    </div>
                    @endcan
                </form>
            </div>
            ...
            $(document).ready(function(){
                $('body').on('click', '.sendQuestionResponse', function(){
                    ...
                    if(userConfirmation ){
                        ...
                        $.ajax({
                            url: "{{ route('prastav-suchana.saveSingleResponse') }}",
                            type: 'POST',
                            data: {
                                _token: "{{ csrf_token() }}",
                                id: id,
            ...
            $(document).ready(function(){
                $('body').on('click', '.acceptQuestion', function(){
                    ...
                    if(userConfirmation ){
                        ...
                        $.ajax({
                            url: "{{ route('prastav-suchana.acceptQuetionByMayor') }}",
                            type: 'POST',
                            data: {
                                _token: "{{ csrf_token() }}",
                                id: id
            ...
            $(document).ready(function(){
                $('body').on('click', '.sendQuestion', function(){
                    ...
                    if(userConfirmation ){
                        ...
                        $.ajax({
                            url: "{{ route('prastav-suchana.sendQuestion') }}",
                            type: 'POST',
                            data: {
                                _token: "{{ csrf_token() }}",
                                id: id
